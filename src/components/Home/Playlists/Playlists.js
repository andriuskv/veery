...
import { dispatchCustomEvent, fileSize } from "../../../utils.js";
...
function Playlist({ playlist, youtube, setYoutube }) {
  ...
  async function clearCache() {
    const { clearStore } = await import("services/db");

    setCacheSize(null);
    await clearStore("artworks");
    await clearStore("metadata");
  }

  async function calcCacheSize() {
    const storage = await navigator.storage.estimate();
    setCacheSize(fileSize(storage.usage));
  }
    ...
    <div className="playlist-entry" key={playlist.id}>
      ...
      <div className="playlist-entry-content">
        ...
        <div className="playlist-entry-bottom">
          ...
          <Dropdown container={{ className: "playlist-entry-dropdown-container" }} onOpen={calcCacheSize}>
            ...
            {playlist.id === "local-files" ? (
              <button className="btn text-btn dropdown-btn playlist-entry-cache-btn" onClick={clearCache}>
                <span>Clear Cache</span>
                {cacheSize ? <span className="playlist-entry-cache-size">({cacheSize})</span> : null}
              </button>
            ) : null}
            {playlist.url ? (
              <>
